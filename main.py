import os import requests from datetime import datetime from time import sleep

=== Konfigurasi dari Railway ===

FINNHUB_KEY = os.getenv("FINNHUB_KEY") BOT_TOKEN = os.getenv("BOT_TOKEN") CHAT_ID = os.getenv("CHAT_ID")

Ambil data 1M XAUUSD (5 candle terakhir)

def get_xauusd_1m(): url = "https://finnhub.io/api/v1/forex/candle" params = { "symbol": "OANDA:XAU_USD", "resolution": "1", "count": 5, "token": FINNHUB_KEY } response = requests.get(url, params=params) return response.json()

Kirim alert ke Telegram

def send_alert(message): url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage" payload = {"chat_id": CHAT_ID, "text": message} requests.post(url, data=payload)

Cek apakah terjadi sweep liquidity (candle terakhir break low/high sebelumnya)

def detect_liquidity_sweep(data): h, l = data["h"], data["l"] if len(h) < 2: return False, ""
...
def detect_choch(data): o, c = data["o"], data["c"] if len(o) < 3: return False
...
def is_killzone(): now_utc = datetime.utcnow() hour = now_utc.hour return (6 <= hour < 8) or (12 <= hour < 14)  # WIB = UTC+7

Loop utama

if name == "main": while True: try: if is_killzone(): data = get_xauusd_1m() if data["s"] != "ok": print("❌ Data error dari Finnhub") continue
